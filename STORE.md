...
### `DELETE` — 0 allocs para lookup, libera Node
...
## 8. Comparación con el `ConcurrentDictionary` anterior

| | Antes | Ahora |
|---|---|---|
| **GET** | `ToArray()` → 1 alloc | 0 alloc |
| **SET** | `ToArray()` + `value.ToArray()` = 2 | `new Node()` + `value.ToArray()` + `NativeMemory.Alloc` ≈ 3 (pero Node y NativeMemory son más baratos que byte[] para el GC) |
| **EXISTS** | 1 alloc | 0 alloc |
| **DEL** | 1 alloc | 0 alloc |
| **INCR** | 1 alloc + string parseos | 0 alloc + string parseos |
