...
- **`StoreEntry` como `struct`**: cada entry vive inline en el `Node`, sin objeto heap separado. Con 100k keys, son 100k objetos menos que el GC no tiene que barrer.
- **Enteros sin strings**: `INCR`/`DECR` parsean y escriben enteros desde `ReadOnlySpan<byte>`, sin alocar `string`. `ToBytes` escribe dígitos directo (1 alloc), `TryParseLong` lee desde span (0 allocs).
